...
class Moving_card(object):
    ...
    def clicking_up(self,deck_list):
        ...
        if len(self.moved_card) > 0:
            for item in deck_list:      
                if not isinstance(item,Deck_two):
                    if item.check_position() and item.checking_card(self.moved_card):
                        ...
                        if isinstance(self.cards,Deck_one):
    ...
    def display_score(self,screen):
         ...
         self.score_value = self.font.render("Score :"+ str(self.score), True, (255,255,255))
         screen.blit(self.score_value,(10,530))
         
    def file_handling(self,score):    
...
class Deck_one(Deck):
    ...
    def draw_the_card(self,screen,card_dict):
        ...
        pygame.draw.rect(screen,Green,[self.rect.left,self.rect.top,71,96],2)
        i = self.y
        if len(self.hidden) > 0:
            for item in self.hidden:
                pygame.draw.rect(screen,Blue,[self.rect.left,i,71,96])
                pygame.draw.rect(screen,Green,[self.rect.left,i,71,96],2)
                i += 32
        if len(self.cards) > 0:
    ...
    def clicking_down(self,card):
        ...
        if len(self.cards) > 0: #at least select one card then the condition is true
            top = self.rect.top
            lst = []
    ...
    def checking_card(self,moved_card):
        ...
        if len(self.cards) == 0:
            ...
            if "hearts" in card or "diamonds" in card:
                if "spades" in self.cards[-1] or "clubs" in self.cards[-1]: # condition for black cards
                    next_card = "X"
                    ...
                    if next_card in card:
                        result = True
            elif "hearts" in self.cards[-1] or "diamonds" in self.cards[-1]: # condition for red cards
                next_card = "X"
                if "king" in self.cards[-1]:
                    next_card = "queen"
                elif "queen" in self.cards[-1]:
...
class Deck_two(Deck):
    ...
    def draw_the_card(self,screen,card_dict):
        ...
        x = self.x
        if len(self.hidden_cards) > 0:
            pygame.draw.rect(screen,Blue,[30,30,71,96])
            pygame.draw.rect(screen,Green_1,[30,30,71,96],2)
...
class Deck_three(Deck):
    ...
    def draw_the_card(self,screen,card_dict):
        #This will draw all the cards on the screen
        pygame.draw.rect(screen,Green_1,[self.rect.left,self.rect.top,71,96],2)
        if len(self.cards) > 0:
...
def main():
    ...
    deck_list = [Deck_two(130,30),Deck_one(30,160),Deck_one(130,160),Deck_one(230,160),
                 Deck_one(330,160),Deck_one(430,160),Deck_one(530,160),
                 Deck_one(630,160),Deck_three(330,30),Deck_three(430,30),Deck_three(530,30),
                 Deck_three(630,30)]
    m_card = Moving_card()
    ...
    text = font.render("Congratulations, You Won!",True,Green_1)
    # -------- Main Program Loop -----------
    ...
    while not done:
        ...
        for event in pygame.event.get(): # User did something
            if event.type == pygame.QUIT:# If user clicked close
                m_card.file_handling(m_card.score) #saving the score
